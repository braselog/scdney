...
train_scClassify <- function(exprsMat_train,
  ...
  ### TODO:
  ### 1. Support sparse matrix
  ### 2. Implement the feature Hierachy (weighted feature hierarchy selection)
  ...
  centroidMat <- do.call(cbind, lapply(unique(cellTypes_train),
                                       function(x) rowMeans(as.matrix(exprsMat_train[de, cellTypes_train == x]))))
    ...
    if (length(unique(cutree_list[[i]])) != 1) {
      ...
      for (j in unique(cutree_list[[i - 1]])) {
        ...
        if (length(unique(trainClass)) != 1) {
          ...
          model[[j]] <- list(train = t(exprsMat[na.omit(hvg[[j]]), trainIdx, drop = FALSE]),
