...
getEnsembleRes <- function(testRes, exclude = NULL, weighted_ensemble = TRUE) {


  # Get the weighted
  if (weighted_ensemble) {
    if (!is.null(exclude)) {
      keep_method <- !grepl(paste(exclude, collapse = "|"), names(testRes$train))
      testRes$train <- testRes$train[keep_method]
      weight <- weight[keep_method]
    }

    errClass_train <- do.call(rbind, lapply(testRes$train, function(x) table(x$classifyRes)/length(x$classifyRes)))
